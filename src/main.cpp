...
void createDemoEntities(Registry& registry, Point window_size) {
    const int n = 5;
    auto dx = window_size.x / 5;
    auto dy = window_size.y / 5;
    for (int i = 0; i < n; ++i) {
        auto triangle = Triangle::create(registry, {{{0, 0}, {100, 0}, {50, 50}}});
        TransformSystem::setPosition(registry, triangle, {i*dx, i*dy});
        AnimationSystem::animateEntity(registry, triangle);

        auto rectangle = Rectangle::create(registry, 75, 50);
...
int main() {
    sf::RenderWindow window(sf::VideoMode({800, 600}), "Shapes demo");
    window.setFramerateLimit(30);

    Registry registry;
    RenderSystem render_system(window);
    createDemoEntities(registry, {800, 600});
    ...
    while (window.isOpen())
    {
        ...
        AnimationSystem::update(registry, {800, 600});
